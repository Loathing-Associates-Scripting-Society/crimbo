import <scripts/crimbo/crimbo21.ash>
...
void handleSetting(string type, int x)
{
	...
	switch(type)
	{
	case "crimbo":			color = "#6FE26F";		break;
	case "crimbo21":		color = "#8BDCE7";		break;
...
void write_settings_key()
{
	...
	writeln("<tr bgcolor=#6FE26F><td>crimbo shared settings for all years</td></tr>");
	writeln("<tr bgcolor=#8BDCE7><td>crimbo21.ash script to automate crimbo 2021</td></tr>");
	writeln("</table>");
}
...
void main()
{
	...
	file_to_map("crimbo_settings.txt", s);
	fields = form_fields();
	if(count(fields) > 0)
	{
		foreach x in fields
	...
	foreach x in s["crimbo"]
	{
		handleSetting("crimbo", x);
	}
	foreach x in s["crimbo21"]
	{
		handleSetting("crimbo21", x);
	}
